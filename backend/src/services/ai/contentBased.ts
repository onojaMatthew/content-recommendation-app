...
export class ContentBasedFiltering {
 private model: tf.LayersModel;
  ...
  //       // Normalized text features
  //       Math.min(content.title.length / 100, 1),
  //       Math.min(content.textContent.length / 500, 1),
  //       Math.min(content.tags.length / 10, 1),
  ...
  //     const textWord = content.textContent.toLowerCase().split(/\s+/);
  //     textWord.forEach(word => {
  //       const hash = (this.hashString(word) % 30 ) + 60;
  //       features[hash] += 1;
  //     })
  //   }
  ...
  //   if (content.type) {
  //     const categoryHash = this.hashString(content.type) % 10; // 10 features for categories
